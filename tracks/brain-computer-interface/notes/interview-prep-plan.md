...
- **运动想象**：C3（右手手区）、C4（左手手区）、Cz（足部）
- **SSVEP**：O1/O2/Oz（枕叶视觉皮层）
- **T7/T8** = 颞叶中段（听觉皮层），旧命名 T3/T4
- **P7/P8** = 颞叶后段，旧命名 T5/T6
- **F7/F8** = 颞叶前段，EOG 监控常用位
- 标准 10-20 系统共 **21 个电极**，耳垂/乳突电极（A1/A2、M1/M2）是参考电极，不计入
...
CSP 核心思想：
  ...
  3. 取 λ 最大的 k 个特征向量 → W1
  4. 取 λ 最小的 k 个特征向量 → W2
  5. 拼接 [W1; W2] 得到完整的 CSP 投影矩阵
  6. 投影后提取 log-方差作为特征 → 输入 LDA 分类器
```

**关键记住**：CSP 本质是找到两类在空间上的方差差异。CSP + LDA 是 MI-BCI 的经典 pipeline。

### 实现（1.5h）

在 `bci/decoder/csp.py` 中实现 `CSPDecoder`，封装 `mne.decoding.CSP` + StandardScaler + LDA pipeline。

### 验证
- [ ] `pytest bci/tests/test_decoder.py -k csp` 通过
- [ ] 能在 PhysioNet MI 数据上跑 CSP + LDA 并得到 >0.8 的 CV 准确率

---

## Day 4：STAR 故事准备

准备 3 个 STAR（Situation-Task-Action-Result）故事，每个控制在 2 分钟内。

### 故事 1：Snakemake 自研框架

- **S**：生信流水线用 Snakemake，rule 语法写死了模块依赖，无法灵活组合
- **T**：需要一个可动态拼接流程的框架
- **A**：用 Python 对象动态生成 rule（MyRule/RuleSet），YAML 配置驱动 DAG
- **R**：30 个分析模块可任意组合，改流程只改一行 YAML

### 故事 2：BCI Pipeline + Transformer 解码器
...
- **S**：rnaseq 流水线跑完需要记录哪些数据可用、哪些不可用
- **T**：一个记录 GSE 数据可用性的后端 + GeneCards 缓存
- **A**：FastAPI 自动发现路由 + 三层配置 + 版本驱动缓存失效
- **R**：4 个 API 端点的轻量后端，与流水线共享 SQLite 数据库
...
| 知识点 | 要点 | 时间 |
|--------|------|------|
| 信息传输率（ITR） | ITR = (log₂(N) + P·log₂(P) + (1-P)·log₂((1-P)/(N-1))) × 60/T | 20min |
| BCI Competition 数据集 | 4 届，III/IVa（MI）、II（P300）、IV 2a（MI 4类） | 30min |
| ERP 成分 | P300(~300ms, oddball)、N100/P200(听觉/视觉)、ERD/ERS(MI) | 30min |
| EEGNet | 深度可分离卷积、紧凑参数、适合小数据 | 20min |
| 10-20/LSL/伪迹 快速复习 | 过一遍 Day1-2 的验收项 | 20min |

### Day 6：模拟面试（被问+自述）

- **Self-intro**（2min）：从生信转向科学数据工程→BCI 项目→工程化能力
- **白板题**：手画 BCI Pipeline 架构图 + 解释每个模块
- **信号处理**：什么是因果滤波？什么时候用零相位？在线和离线为什么要两套引擎？
- **Transformer**：为什么 RoPE、为什么因果注意力、GPT vs BERT 差别
- **CSP**：CSP 的原理、为什么 CSP + LDA 是 MI 经典
